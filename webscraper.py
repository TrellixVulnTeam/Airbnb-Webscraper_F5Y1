...
class Webscraping:
    def __init__(self, start_date: datetime.date, end_date: datetime.date, number_of_adults: int, maximum_price: int):
        # self._driver = webdriver.Chrome(ChromeDriverManager().install())  # THIS IS PROB DEF A BAD IDEA -RUSH
        self.start_date = start_date
        self.end_date = end_date
        self.num_adults = number_of_adults
    ...
    def start_search(self):
        ...
        params = {
            ...
            "checkout": self.end_date.strftime('%Y-%m-%d'),
            "adults": self.num_adults,
            "price_max": self.max_price
        }
        querystring = parse.urlencode(params)
        locations = {
            'Maine': "https://www.airbnb.com/s/Maine--United-States/homes" + "?" + querystring,
            'Seattle': "https://www.airbnb.com/s/Downtown-Seattle--Seattle--WA/homes" + "?" + querystring,
            'New Hampshire': "https://www.airbnb.com/s/New-Hampshire--United-States/homes" + "?" + querystring,
        }

        for location_name, web_address in locations.items():
            # self._driver.get(web_address)
            page = requests.get(web_address, headers={'User-agent': 'Super Bot Power Level Over 9000'})
            soup = BeautifulSoup(page.content, 'html.parser')
            soup.prettify()
            listings = soup.find_all("div", {"itemprop": "itemListElement"})
            ...
            for listing in listings:
                ...
                str_total_cost = listing.find("div", {"class": "_tt122m"}).getText()
                int_total_cost = int(str_total_cost[1:-6].replace(',', ''))
                ...
                properties[name] = (location_name, int_total_cost, num_bedrooms, num_beds)

        df = pd.DataFrame.from_dict(properties, orient='Index', columns=["Location", "Total Cost", "# of Bedrooms", "# of Beds"])
        df.index.name = 'Property Name'
        df.reset_index(inplace=True)
        return df
...
def main():
    webscraper = Webscraping(
        start_date=datetime.date.fromisoformat('2022-06-14'),
        end_date=datetime.date.fromisoformat('2022-06-17'),
        number_of_adults=4,
        maximum_price=300
    )
    result = webscraper.start_search()
    print(result)
